...
const deleteGroup = util.errHandler(async (req, res) => {
  try {
    let checkRecordId=await knex('artisan_group').where({id:req.query.id})
...
let editCategory = util.errHandler(async (req, res) => {
  try {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      let checkId=await knex('category').where({id:id})
...
const addSubCategory = util.errHandler(async (req, res) => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      let checkId = await knex('category').where({id:parentId});

      console.log(checkId,"checkId")

      if (!checkId.length) {

        res.send({ code: util.statusCode.FOUR_ZERO_ONE, message: "Invalid ParentId" })
  ...
  try {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      if (!checkId.length) {
        ...
        res.send({ code: util.statusCode.FOUR_ZERO_ONE, message: "Invalid Id" })
      ...
      if (files.banner && files.banner.length > 0) {
        ...
        let fileExtension2 = files.banner[0].originalFilename.replace(/^.*\./, "");
        if(fileExtension2==="webp"){
          uploadedFiles2 = await uploadCloud.upload_infographics(files.banner);
        }else{
          uploadedFiles2 = await new Promise((resolve, reject) => uploadCloud.upload_image2(files.banner, resolve, reject));
        }
        criteria['banner_image'] = uploadedFiles[0]
      }
  ...
  try {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      if (files.files && files.files.length > 0) {
       ...
       resData = await admin_dao.importProduct(files.files[0].path, req.admin.id);
      } else {
        res.send({ code: util.statusCode.FOUR_ZERO_ZERO, message: util.statusMessage.CSV_FILE_CHOOSE });
      }
      res.send({ code, message,resData:resData });
    })
  } catch (error) {
    res.send({ code: util.statusCode.FOUR_ZERO_ZERO, message: error })
  }
...
const updateTestimonialsPost = util.errHandler(async (req, res) => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      let fileExtension = files.imgs[0].originalFilename.replace(/^.*\./, "");
      if(fileExtension==="webp"){
        uploadedFiles = await uploadCloud.upload_infographics(files.imgs);
      }else{
        uploadedFiles = await new Promise((resolve, reject) => uploadCloud.upload_image2(files.imgs, resolve, reject));
      }
...
// ***********   deleteProductImage  ************/
const deleteProductImage = util.errHandler(async (req, res) => {
  try {

    let code = util.statusCode.OK;
    let message = util.statusMessage.SUCCESS;

    await knex.raw(`delete from productImage where id='${req.body.id}'`);
    res.send({ code,message});
