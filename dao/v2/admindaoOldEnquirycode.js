...
const getSubCategory = (categoryId) => {
  return new Promise((resolve, reject) => {
    ...
    FROM category  AS c WHERE deleted='1' AND isActive='1'  AND parentId=${categoryId} order by id desc`;
...
const getSubSubCategory = (categoryId) => {
    return new Promise((resolve, reject) => {
      let query = `SELECT c.* ,(SELECT COUNT(id) FROM products WHERE deleted='1' AND c.id=subsubcategoryId) AS totalProduct,
      (SELECT COUNT(id) FROM category  AS cc WHERE deleted='1' AND cc.parentId=c.id) AS totalsubCategoryId
      FROM category  AS c WHERE deleted='1' AND isActive='1' AND parentId=${categoryId} order by id desc`;
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      query = `SELECT e.id,uniqueId,productId,title,description,typeofEnquiry,isGenrate,ud.name,u.email,pproductId,
            mailBy,mailSubject,mailBody,e.created_at from enquiries as e
...
const genrateNewEnquiry = async (criteria, adminId) => {
  // try {
    if (criteria.id && criteria.id != "null") {
      ...
      console.log("criteria",criteria,"updated_at", new Date())

      await Enquiries.query().findById(id).patch(criteria).debug();
...
const broadCastEnquiry = async (criteria) => {
  ...
  console.log("criteria=?????broadCastEnquiry",criteria)
  ...
  try {
    if (criteria.requestTo === "All") {
      ...
      artisan = await knex("artisan_group")
        .select("total_artisan")
        .where({ id: criteria.requestTo })

      if (artisan) {

        const artisanArr = artisan[0]["total_artisan"].split(",");

        artisanArr.map(async (UserId) => {

          let checkUser=await knex("users").where({id:UserId}).where("is_verified","1").where('appToken',"!=","");
          ...
          let token=await knex("users").where({id:UserId}).where("is_verified", "1")
          ...
          if(!_.isEmpty(checkUser)){
              ...
              util.sendNotif(notificationPayload)
              await knex('notifications').insert(notificationPayload)
              emitSocket('receiveEnquiry', socketResponse, UserId)
            }
          }
        });
...
const broadCastEnquiry2 = async (criteria) => {
  ...
  try {
      ...
      await knex("chat").insert(chatOb).debug()
...
const importArtisan = async (filePath) => {
  try {
    ...
    // recipients.forEach(async (criteria) => {
      ...
      const checkMobile = await knex("users").where("isActive", "1").where("role","2").where("mobile", criteria.Mobile).debug()
      const checkEmail = await knex("users").where("isActive", "1").where("role","2").where("email", criteria.Email).debug()
  ...
  try {
    ...
    fs.createReadStream(filePath)
      ...
      .on("data", async (row) => {
        ...
        // csvData["name"] = row.ProductName;
        // csvData["amount"] = row.Amount ? Number(row.Amount):0;
        // csvData["inventoryQty"] = row.InventoryQty ? Number(row.InventoryQty):0;
        // csvData["doableQty"] = row.MinQty? Number(row.MinQty):0;
        // csvData["searchTags"] = row.Tags?row.Tags:'';
        // csvData["description"] = row.Description ? row.Description:'';
        // csvData["userId"] = userId;
        // csvData["isActive"] = "1";
        // csvData["verified"] = "1";
        // csvData["vendorName"] = row.VendorName?row.VendorName:'';
        // csvData["sku"] = row.SKU ? row.SKU :'';
        // csvData["categoryId"] = row.CategoryName ? await getCatgoryId(row.CategoryName):2;
        // csvData["subcategoryId"] = row.SubCategoryName ? await getCatgoryId(row.SubCategoryName) :0;
        ...
        if(!_.isEmpty(checkUser)){
              ...
              "categoryId":row.CategoryName ? await getCatgoryId(row.CategoryName):2,
              "subcategoryId":row.SubCategoryName ? await getCatgoryId(row.SubCategoryName) :"0",
              "subsubcategoryId":row.SubSubCategoryName ? await getCatgoryId(row.SubCategoryName) :"0",
...
//   try {
    ...
    //     // let lastId=await knex("products").insert(csvData)
...
const editStoriesPut = async (criteria) => {
  try {
    const { title, description, image } = criteria
...
const getExhibannerList = async (userId) => {
  try {
    return new Promise((resolve) => {
      ...
      let query = `SELECT id,type,description,title,link,created_at,isActive,(select count(id) from user_exhibition where exhibitionId=cms.id) as totalUser,name FROM cms WHERE type = 'Exhibition'`
